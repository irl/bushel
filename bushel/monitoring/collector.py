...
@nagios_check
def check_collector_index_created() -> NagiosResponse:
    if len(sys.argv) != 2:
...
def check_collector_latest_recent(name: str, path: str) -> NagiosResponse:
    if len(sys.argv) != 2:
        raise RuntimeError("No hostname specified.")
...
@nagios_check
def check_collector_latest_recent_bridge_server_descriptor() -> NagiosResponse:
    return check_collector_latest_recent("bridge_server_descriptors", "recent/bridge-descriptors/server-descriptors")

@nagios_check
def check_collector_latest_recent_bridge_extra_info() -> NagiosResponse:
